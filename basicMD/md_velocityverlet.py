...
#Updates particle velocites based on force array and updates particle
#coordinates based on velocity array using Velocity Verlet algorithm
def run_verlet(rounds, timestep):
    global force
    global velocity
    global coords
    ...
    for x in range(rounds):
        old_accel = new_accel
        coords += velocity * timestep + 0.5 * old_accel * timestep * timestep
        checkbounds()
        cforceenergy()
        new_accel = force/mass
        velocity += 0.5 * (old_accel + new_accel) * timestep
#======END===================================================================#
...
def run(roundspergroup, timestep, groups):
    ...
    for x in range(groups):
        ...
        run_verlet(roundspergroup, timestep)
