...
def outputdata(ctime):
    ...
    output.write("%-.9e  %-.9e  %-.9e  %-.9e\n" % \
                 (ctime, kenergy, ctemperature, cpressure))
...
#Updates particle coordinates based on velocity array using Euler integration
...
#To run simulation with Berendsen thermostat on, uncomment indicated
...
def run(roundspergroup, timestep, groups):
    ...
    for x in range(groups):
        ...
        outputpdb()
        cpressure = (1.380649)*(numberofatoms)*(ctemperature)*((10**7)/((boxsize[0]*boxsize[1]*boxsize[2])))
        outputdata(currenttime)
        print("Time: %.3e  |  KEnergy: %.3e  |  Temp: %.3e  |  Pressure: %.3e" % \
              (currenttime, kenergy, ctemperature, cpressure))
